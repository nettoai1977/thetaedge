...
- **2026-09-06 (goal round 10) — SECOND-ORDER CORRECTION: the round-8 '11–20pp underpricing at T−120' was itself mostly lookahead leakage.** Canonical v3 semantics decoded: P(60t) = the CLOSE of the bar ENDING at 60t (i.e. P(180) = close(bar w+120)) — knowable at decision time. My offline scripts had used close(w+180) (one minute of future information at T−120) and live sigma included the not-yet-closed signal bar. Fixed in all three scripts (backfill_analysis, timing_analysis, analyze). Leak-free results: **the market is approximately FAIR at T−120 in the tradable bins** (0.70–0.80 → 75.9% vs 75%; 0.80–0.90 → 86.6% vs 85%; 0.90–0.95 → 91.7% vs 92.5%; 0.95+ → 99.6% vs 97.5%), with the residual realized-vs-price gap of **+1–2pp in high bins = exactly G1's measured edge** (market prices ≈ Brownian; realized beats both). Gated set: 615 windows, EV>0 37.6%, median EV −$0.016, mean +$0.079, realized wr 90.6% (within G1 bounds). Low bins (0.50–0.70) slightly OVERpriced on momentum (48.5–57% realized) — noted, outside frozen gate scope. Live-vs-offline FV parity: mean −0.056 (live mids vs bar closes; live is higher-fidelity). Also this round: live sigma source switched to REST 1m bar closes (day-span fetch from UTC midnight; mid-marks ran ~2x tight and inflated live FV), funding capture live (premiumIndex poll), sigma provenance now stamped in every ledger row. Standing lesson recorded: EVERY offline-vs-live discrepancy must be traced to a construct difference before trusting either — this session's FV gap led to discovering the leakage.
- **2026-09-06 (goal round 13) — KILL-CRITERION MONITOR FLAGGED: t=3 calibration edge ≈ 0 on the last 30 days.** New premise-robustness stress grid (`reports/premise_stress.md`, DIAGNOSTIC — frozen gates untouched). Part A (signal layer, 30d of Binance 1m bars, canonical v3 boundary, gated FV≥0.80): the frozen construction (t=3, σ-lookback 60, ties=loss) delivers realized-wr-minus-G1-bin-wr = **−0.70pp (n=3,820, z≈−1.6 — below zero but not decisively negative)**; robustness across the grid: σ-lookback 30→−1.03pp, 120→+0.09pp, ties=win→−0.52pp, t=4 → **+0.88..+1.20pp (z≈2.4, the only robustly positive cell)**. Per the pre-registered monitor (G1 gap must stay >0 on rolling 30d), **the t=3 premise edge is currently NOT confirmed**; consistent with G1's own warning that 2026 drift is thinner (77.4%, gap +0.4pp). Consequences, per protocol: (1) gates NOT moved, spec NOT revised — measurement continues exactly as frozen; (2) the 2–4 week G2 verdict will weigh this: if the 30d compression persists, the premise verdict is NO-GO even if the book were cheap; (3) t=4's stronger edge is recorded as a roadmap note for a possible formal spec revision AFTER the G2 verdict — not acted on now. Part B (market layer, upper bound of an upper bound): leader-side last-print ≤ T−120 on 856 gated windows: EV>0 95.3%, mean +$0.1236 at frozen fee (fee tier 0.10 immaterial: −0.4¢) — but **median print staleness 47s, only 0.8% fresher than 30s**, quantifying why offline EV cannot be taken at face value; live ask-side gates remain the only decisive measurement.
